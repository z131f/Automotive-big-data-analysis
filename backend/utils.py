...
def create_hive_table(table_name, schema, config):
    ...
    try:
        ...
        drop_sql = f'DROP TABLE IF EXISTS {table_name}'  # Use f-string for table_name
...
def insert_into_hive_table(table_name, data, schema, config):
    """
    將數據插入到 Hive 表中，适配 car_data 表結構，並處理 ARRAY 和 MAP 類型。
    此版本要求調用者將 MAP 類型數據格式化為 Python 字典，
    並在內部將其轉換為 Hive 的 map(...) 字符串字面量，並正確處理字符串轉義。
        ...
        for row_dict in data:
            ...
            for col_name in columns:
                ...
                if value is None:
                    ...
                    formatted_items = []
                    for k, v in value.items():
                        # Keys are STRING, values are INT in this schema
                        # Escape single quotes in keys, then quote them
                        escaped_k = str(k).replace("'", "''")

                        # Values are INT, so they should NOT be quoted, just converted to string
                        formatted_v = str(v)

                        formatted_items.append(f"'{escaped_k}', {formatted_v}")

                    if formatted_items:
                        row_values_formatted.append(f"map({', '.join(formatted_items)})")
                    else:
                        row_values_formatted.append("map()")  # Handle empty map
                elif isinstance(value, str):
                    # For regular strings, escape single quotes and wrap in single quotes
                    escaped_value = value.replace("'", "''")
                    row_values_formatted.append(f"'{escaped_value}'")
                else:
                    # For numbers, floats, etc., convert to string directly
                    row_values_formatted.append(str(value))
